...
# (2020-10-01)
# - Add infrastructure required for overriding input/output encoding
...
# 2.2: (2023-05-29)
# - import narrow font helpers from Bly's fork featured in the skill system
#   Bly's edit was first released on 2020-07-22, and was merged into the skill system on 2021-10-25
...
def escape_nonascii(s):
	"""Encode every non-ASCII char as its UTF-8 bytes in [0xNN] form so
	ParseFile (which rejects raw UTF-8) receives pure byte escapes. ASCII
	and existing [..] control codes pass through untouched.

	Continuation bytes that would be 0x80 are emitted as 0xC0 instead. The
	world-map narration text path drops raw 0x80 bytes while copying the
	string into RAM, which desynced the decoder and garbled the syllable
	(verified: ROM held `ec 9d 80` for 은, the RAM buffer held only `ec 9d`).
	KRTextEngine's UTF8_Decode masks continuation bytes with & 0x3F, and
	0xC0 & 0x3F == 0x80 & 0x3F == 0, so the decoded codepoint is identical.
	Lead bytes are never 0x80, so every 0x80 we emit is a continuation byte.
	"""
	out = []
	for ch in s:
		if ord(ch) < 0x80:
			...
			for b in ch.encode('utf-8'):
				out.append('[0x%02X]' % (0xC0 if b == 0x80 else b))
	return ''.join(out)
...
def main(args):
	...
	try:
		...
		for entry in generate_text_entries(Preprocessor(verbose).preprocess(inputPath), verbose): # create separate files for each text entry
			...
			entry.text = escape_nonascii(entry.text)
