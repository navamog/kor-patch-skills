...
CLUT은 texture와 별개의 VRAM 자원이므로 저장 형태도 분리될 수 있다. TIM은 CLUT 없는 pixel-only와 pixel 없는 CLUT-only를 모두 표현하며, CLUT block의 선언 크기는 하나의 palette가 아니라 **palette 여러 벌**일 수 있다(4bpp는 16 entry, 8bpp는 256 entry가 한 벌). 한 그림을 구획으로 나눠 구획마다 다른 CLUT을 지정하면 4bpp texture도 16색을 넘는 화면을 낼 수 있으므로, 같은 index의 표시 색이 위치에 따라 다르면 archive 안의 별도 CLUT 자산과 그 구획 대응을 먼저 확인한다. `references/strategy/graphics-text.md` §2의 팔레트 판정을 적용한다.
