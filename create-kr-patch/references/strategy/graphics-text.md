...
인덱스 색 자산에서 같은 인덱스가 자리마다 다른 색으로 표시되면, 색이 런타임에 정해진다고 판정하기 전에 **팔레트가 편집 가능한 저장 자산인지** 확인한다. 하나의 그림이 팔레트를 여럿 가지고 영역마다 다른 팔레트로 소비되는 구성이 있으며, 이때 인덱스→색 대응은 위치의 함수다. 이를 런타임 결정으로 오판하면 남은 자유도가 인덱스 선택뿐이라고 보고 그 안에서만 최적화하게 되는데, 그 전제에서는 어떤 선택도 균일한 색을 내지 못한다. 자산 자체의 팔레트가 무의미한 램프이거나 표시 색과 무관하다는 사실은 팔레트가 없다는 근거가 아니라 **다른 곳에 있다**는 단서다.

이 판정은 인덱스 후보를 비교해서 내리지 않고, 팔레트 후보에 식별 가능한 값을 넣어 실제 소비 경로에서 관측해 내린다. 후보마다 구별되는 값을 넣으면 팔레트–영역 대응과 그 격자 경계, 그리고 저장 값이 표시까지 도달하며 보정되는지가 한 번의 관측으로 함께 드러난다.

팔레트를 편집할 수 있으면 수정 가능 자원은 인덱스가 아니라 색이다. 이때 보호 조건은 팔레트 항목 단위로 선언한다. 영역별로 보존 대상 픽셀이 실제로 소비하지 않는 항목만 사용하고, 가져간 항목을 쓰던 픽셀은 그 영역의 원본 팔레트를 기준으로 다시 배정하며, 재배정이 보호 영역에 남긴 변화량을 계량해 허용 범위를 판정한다.
